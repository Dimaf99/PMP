...
public class GameFragment extends BaseFragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        main = new MainGameFragment(view);
    ...
    public class MainGameFragment extends Thread{
        private View view;
        ...
        @Override
        public void run() {
            CompletableFuture<Settings> future = CompletableFuture.supplyAsync(() -> {
                try {
                    Thread.sleep(5000);
            ...
            try {
                Settings settings = future.get();
                min = settings.getMin();
                max = settings.getMax();
                time = settings.getTime();
                arrWords = settings.getWords();
        ...
        private void configurateWords(){
            // перемешиваем
            char [] words_temp = GameFragment.shuffle( this.getRightAnswer() ).toUpperCase().toCharArray();
            // массив
        ...
        private void drawLetters(int param){
            if ( param == 0){ // word
                LinearLayout lay1 = (LinearLayout) this.view.findViewById(R.id.lay1);
                lay1.removeAllViews();
                ...
                LinearLayout lay2 = (LinearLayout) this.view.findViewById(R.id.lay2);
                lay2.removeAllViews();
        ...
        private String generateWord(){
            for( String name : this.arrWords){
                if ( name.length() >= this.min && name.length() <= this.max ){
                    this._arrWords.add(name);
                }
            }
